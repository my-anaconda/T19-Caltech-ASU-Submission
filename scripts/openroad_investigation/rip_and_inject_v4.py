...
# --- Snap fix: the router's own V1 (VIA_VIA12) placements are excluded from
# injection (V1 must stay frozen - see below), but the M2 wire it drew
# assumes ITS via is there. Snap each such wire endpoint onto the nearest
# ORIGINAL (pristine, untouched) VIA_VIA12 position instead, so the new M2
# wire actually lands on the via that's really there. ---
_via12_re = re.compile(
    r"^cell_Block1\.insert\(pya\.CellInstArray\(cell_VIA_VIA12\.cell_index\(\), "
    r"pya\.Trans\(\d+, (?:True|False), pya\.Vector\((-?\d+), (-?\d+)\)\)\)\)\s*$",
    re.MULTILINE,
)
pristine_via12_def = []
for mm in _via12_re.finditer(text):
    kx, ky = int(mm.group(1)), int(mm.group(2))
    pristine_via12_def.append((kx * 0.25, ky * 0.25))
print(f"Found {len(pristine_via12_def)} pristine VIA_VIA12 positions (DEF units) for snapping")


def _nearest_via12(x, y, max_dist=2000):
    best, best_d2 = None, None
    for (ox, oy) in pristine_via12_def:
        d2 = (ox - x) ** 2 + (oy - y) ** 2
        if best_d2 is None or d2 < best_d2:
            best_d2, best = d2, (ox, oy)
    if best is not None and best_d2 <= max_dist ** 2:
        return (round(best[0]), round(best[1]))
    return None


skipped_v1_positions = {(x, y) for (via_name, x, y) in routed_geo["vias"] if via_name == "VIA_VIA12"}
snap_map = {}
unsnapped = 0
for (x, y) in skipped_v1_positions:
    snapped = _nearest_via12(x, y)
    if snapped is not None:
        snap_map[(x, y)] = snapped
    else:
        unsnapped += 1
print(f"Built snap map for {len(snap_map)}/{len(skipped_v1_positions)} skipped V1 position(s); {unsnapped} had no nearby original within tolerance")

snapped_wires = []
snap_hits = 0
for (layer_num, x1, y1, x2, y2) in routed_geo["wires"]:
    if (x1, y1) in snap_map:
        x1, y1 = snap_map[(x1, y1)]
        snap_hits += 1
    if (x2, y2) in snap_map:
        x2, y2 = snap_map[(x2, y2)]
        snap_hits += 1
    snapped_wires.append((layer_num, x1, y1, x2, y2))
routed_geo["wires"] = snapped_wires
print(f"Snapped {snap_hits} wire endpoint(s) onto original VIA_VIA12 positions")

LNAME_TO_NUM = {"M2": 20, "V1": 21, "M3": 30, "V2": 25, "M4": 40, "V3": 35, "M5": 50, "V4": 45, "M6": 60}
METAL_LAYER_NUMS = {20, 30, 40, 50, 60}
# V1 (layer 21, VIA_VIA12 family) is EXCLUDED here: it carries its own internal
# M1-layer landing-pad shape (confirmed: cell_VIA_VIA12 has a LayerInfo(19,0)
# sub-shape), so touching V1 at all perturbs the "immutable" M1 seed layer
# indirectly. V1 must stay exactly as in the pristine source - only V2 and
# above (M2-M3 vias and up) are genuinely safe to rewrite.
VIA_LAYER_NUMS = {25, 35, 45}
# via type name -> its via layer number, by prefix match (handles suffixed multi-cut variants too)
VIA_TYPE_PREFIX_TO_LAYER = {"VIA_VIA23": 25, "VIA_VIA34": 35, "VIA_VIA45": 45, "VIA_VIA56": 55}
...
for entry in net_shapes:
    ...
    for lname, boxes in entry["shapes_by_layer"].items():
        ...
        if lnum is None or lnum not in old_bboxes_by_layer:
            continue
...
for (via_name, x, y) in routed_geo["vias"]:
    if via_name == "VIA_VIA12":
        # never create a new V1 - it would carry its own M1 landing pad,
        # perturbing the frozen seed layer. The original V1 for this pin
        # stays in place untouched (see strip step above).
        skipped_v1 += 1
        continue
...
print(f"Skipped {skipped_v1} new V1(VIA_VIA12) via(s) from router output (V1 frozen)")
