...
def pin_boxes_for_instance(i, inst):
    """Returns {pin_name: (xlo,ylo,xhi,yhi) in microns, absolute}, excluding VDD/VSS."""
    ct = inst["cell_type"]
...
# Index instances by exact (x,y) origin in microns for subcircuit->instance lookup
inst_by_pos = {}
...
no_pins_available = 0
for net in netlist:
    std_members = []  # (inst_idx, sp_x, sp_y)
    anchors = []  # via/other terminal positions (x,y)
    ...
    for inst_idx in set(std_members):
        boxes = pin_boxes_for_instance(inst_idx, instances[inst_idx])
        if not boxes:
            no_pins_available += 1
            continue
        # nearest pin to any anchor (via position) in this same net; if no
        # anchors (rare), fall back to nearest pin to the instance's own origin
...
with open("/tmp/resolved_nets2.json", "w") as f:
